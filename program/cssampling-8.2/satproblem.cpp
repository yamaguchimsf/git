...
void SatProblem::parseSliFile(char* inputFile, int size)
{
  int line = 0;
  int idx = 0;
  char delims[] = ",";
  ...
  trd = new int*[datasize_l];
  if (trd == NULL){ cout << "cannot allocate memory"<< endl; exit(1);}
  for(int i = 0; i < datasize_l; i++){
    trd[i] = new int[numVariables_];
    if (trd[i] == NULL){ cout << "cannot allocate memory"<< endl; exit(1);}
  }
  for(int i = 0; i < datasize_l; i++){
  ...
  if (!ifile_){
    ...
    while (ifile_.good()){
      ...
      token_ = strtok(buffer_, delims);
      while(token_ != NULL){
        trd[line][idx] = atoi(token_);
        token_ = strtok(NULL, delims);
        idx++;
      }
      idx = 0;
...
void SatProblem::calcQ(int size)
{
  int idx;
  datasize_t = 0;
  ...
  trs = new int*[datasize_t];
  for(int j = 0; j < datasize_t; j++){
    trs[j] = new int[numVariables_];
  }
  for(int j = 0; j < datasize_t; j++){
  ...
  for(int i = 0; i < datasize_t; i++){
    for(int j = 0; j < numVariables_; j++){
      if(trs[i][j] == 1){
        q_xF[i] += log(getVarProbi(j));
      }else{
        q_xF[i] += log(1-getVarProbi(j));
      }
    }
  }
...
void SatProblem::normalize(double* vec, int vecsize)
{
...
bool SatProblem::calcP(int** sample, int size, int datasize, bool csmode)
{
  double* weight   = new double[size];
  double* app_prob = new double[numVariables_];
  double* cnt      = new double[numVariables_];
  int datasize_s   = 0;
  double hd;
  double temp;
  int** sam;
  ...
  // count a num. of different models
  for(int i = 0; i < size; i++){
    if(modelcheck(i, sample)){
      datasize_s++;
    }
  }

  sam = new int*[datasize_s];
  for(int j = 0; j < datasize_s; j++){
    sam[j] = new int[numVariables_];
  }
  for(int j = 0; j < datasize_s; j++){
    for(int k = 0; k < numVariables_; k++){
      sam[j][k] = 0;
  ...
  if (csmode){
    for(int i = 0; i < size; i++){
      for(int j = 0; j < numVariables_; j++){
        cnt[j] += (double)sample[i][j];
      }
    }
    for(int j = 0; j < numVariables_; j++){
      app_prob[j] = cnt[j]/(double)size;
      //printf("%.6f,",app_prob[j]);
    }
  }else{
    for(int i = 0; i < size; i++){
      for(int j = 0; j < numVariables_; j++){
    ...
    normalize(weight, size);

    for(int i = 0; i < size; i++){
      for(int j = 0; j < numVariables_; j++){
        cnt[j] += weight[i] * (double)sample[i][j];
      }
    }
  ...
  hd = 0;
  printf("datasize_t=%d,datasize_s=%d: ",datasize_t,datasize_s);
  bool flag = false;
  for(int i = 0; i < datasize_t; i++){
    flag = false;
    for(int j = 0; j < datasize_s; j++){
      if(veccmp(trs[i],sam[j],numVariables_)){
        flag = true;
        //for(int k = 0; k < numVariables_; k++){
          temp = sqrt(q_xF[i]) - sqrt(app_prob[j]);
          if(temp > 1.0e-6){
            hd += temp*temp;
          }
          temp = sqrt(1-q_xF[i]) - sqrt(1-app_prob[j]);
          if(temp > 1.0e-6){
            hd += temp*temp;
          }
          //printf("cp: %f\n",hd);
...
bool SatProblem::veccmp(int* vec1, int* vec2, int size)
{
