...
int main(int argc, char** argv)
{
  ...
  while (( option = getopt(argc, argv, "ionsdprtmuSflO")) != -1)
    switch(option){
    case 'i' :
    ...
    case 'O' :
      ...
      break;
    default :
      cout << "usage of MRG ./mrg [-ionsdprtuSfl]" << endl;
      cout << "-i : set input db file   (default = ./dice.cnf)" << endl;
      cout << "-o : set input prob.file (default = ./dice.prb)" << endl;
      cout << "-n : set walkmax         (default = 100000)" << endl;
      cout << "-s : set numSample       (default = 100)" << endl;
      cout << "-d : set datasize        (default = 100000)" << endl;
  ...
  prbFile.close();
  satproblem->parseSliFile(slifilename, datasize);
  ...
  slisamples = new int* [ssparams->numSample*maxiter];
  for(int j = 0; j < ssparams->numSample*maxiter; j++){
    slisamples[j] = new int[satproblem->getNumVariables()];
  }

  begSec = timer.time();
  for(sa = 0; sa < ssparams->numSample*maxiter; sa++){
    slicesampling->getSample(satproblem, sample);
    for(int j = 0; j < satproblem->getNumVariables(); j++){
      slisamples[sa][j] = sample[j];
    }
    if(sa % ssparams->numSample*maxiter / 10 == 0 && sa > 0){
      if(!satproblem->calcP(slisamples, sa, datasize, csmode)){
        break;
      }
    }
  }
  ...
  slicesampling->calcFixed(ssparams->numSample*maxiter);
  if (cdebug == true){
    cout << "## end SLICE SAMPLING" << endl << flush;
    cout << "## start output " << endl << flush;
  }
  samplesat->calcAve2(maxiter);
  ...
  satproblem2->parseSliFile(slifilename, datasize);
  prbFile.close();
  ...
  samsamples = new int* [ssparams->numSample*maxiter];
  for(int j = 0; j < ssparams->numSample*maxiter; j++){
  ...
  for(sa = 0; sa < ssparams->numSample*maxiter; sa++){
    ...
    if(sa % ssparams->numSample*maxiter / 10 == 0 && sa > 0){
      if(!satproblem2->calcP(samsamples, sa, datasize, csmode)){
        break;
      }
    }
  }
